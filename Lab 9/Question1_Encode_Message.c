...
/*1 December 2020*/
/*Question 1 Lab 9 (Encode a Message)*/
...
int main(void)
{
  ...
  {
    for(j=0;i<MAX_STR_LEN && alpha[j]!= '\0';j++)
    {
        /*If a character is lowercase or is equal to a character in the alphabet, it is replaced with the equivalent character in the permutation array*/
        if(msg[i] >= 'a' && msg[i] <= 'z' && msg[i] == alpha[j])
        {
            msg[i++] = perm[j];       /*Put the jth element of the permutation array into the next position in the message*/
            j=-1;                     /*Set j to -1 so when it increments it will go back to position 0*/
        }
        /*Checking if a character is uppercase and is equal to a character in the alphabet after being changed to lowercase*/
        if(msg[i] < 'a' && msg[i] >= 'A' && (msg[i] + ('a' - 'A') == alpha[j]))
        {
            /*The msg array's character is replaced with the permutation array's uppercase character*/
            msg[i++] = (perm[j] - ('a' - 'A'));
            j=-1;
        }
    }
